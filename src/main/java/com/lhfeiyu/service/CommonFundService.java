	...
	*//**
	...
	private JSONObject updateUserMoney(String payPass, String optType, BigDecimal totalMoney, int userId, String username, JSONObject json, boolean noPassword,String theOtherName){
		...
		if(Check.isNotNull(openId)){
			...
			JSONObject messageJson = Message.buildMsg(openId, Const.wx_moban_2, Const.page_user_center, 
							 "first", firstValue, "tradeType", attr1Value, "curAmount", attr2Value, 
							 "tradeDateTime", dateStr, null, null, "remark", Const.wx_message_remark);
			Message.sendMessage(Const.wx_moban_2, messageJson);//JSONObject resultJson = Message.sendMessage(ConstField.wx_moban_10, messageJson);
		}
		return json;
	}
