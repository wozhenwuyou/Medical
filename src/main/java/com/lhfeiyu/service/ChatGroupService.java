...
/*	@Autowired
    ...
    public static String createGroup(ChatGroup group){
		...
		String subSig = Const.generateSubSig(subAccount, password, timeStamp);
		String url = Const.rl_ytx_base_sub_url+subAccount+"/Group/CreateGroup?sig="+subSig;
		String authorization = Const.getSubAuthorization(subAccount, timeStamp);
    ...
    public static boolean deleteGroup(ChatGroup group){
    	...
    	String timeStamp = new SimpleDateFormat("yyyyMMddHHmmss").format(new Date());
		String subSig = Const.generateSubSig(subAccount, password, timeStamp);
		String url = Const.rl_ytx_base_sub_url+subAccount+"/Group/DeleteGroup?sig="+subSig;
		String authorization = Const.getSubAuthorization(subAccount, timeStamp);
