...
<body>
	...
	<div class="pz_main">
		<div class="w_1000">
			<div class="c_1100_10">
				当前位置：<a href="/">首页</a> - <a href="/consultDoctorList">我要咨询</a>
			</div>
				...
				<div class="l_262 pz_tab">
						...
						<select id="province" class="input8"
							...
							<c:forEach var="provinceCityArea" items="${provinceCityAreaList}">
								<option value="${provinceCityArea.id}"
									<c:if test="${user.province == provinceCityArea.id}">selected = selected</c:if>>${provinceCityArea.areaName}</option>
							</c:forEach>
					...
					<div class="d_262 bd">
							...
							<ul>
								<c:forEach items="${deptList}" var="dept">
									<li><a href="javascript:">${dept.name}</a>
										<ul id="deptUl" class="sub_ny" style="display: block;">
											<c:forEach items="${dept.subDeptList}" var="subDept">
												<li><a id="dept_${subDept.id}" href="javascript:"
													onclick="refreshDoctorList({departmentId:${subDept.id}});">${subDept.name}</a></li>
											</c:forEach>
										</ul></li>
								</c:forEach>
							</ul>
						...
						<div class="one_262">
							<ul>
								<li>
									<%-- <a href="javascript:">${hospital.briefName}</a> --%>
									<ul id="hospitalUl" class="sub_ny" style="display: block;">
										<c:forEach items="${hospitalList}" var="hospital">
											<li><a id="hospital_${hospital.id}" href="javascript:"
												onclick="refreshDoctorList({hospitalId:${hospital.id}});">${hospital.briefName}</a></li>
										</c:forEach>
									</ul>
								</li>
							</ul>
	...
	<script type="text/javascript"
		src="/js/front/domain/user/consultDoctorList.js" title="v"></script>
		...
		{{#rows}}
			<li>
                <table cellpadding="0" cellspacing="0" border="0">
                    <tr height="110">
                        <td width="19">&nbsp;</td>
                        <td width="186" class="pointer" onclick="location.href='/doctor/{{attrStr}}'">
							 <div class="l_71"><img src="{{avatar}}" width="71" height="71" onerror="this.src='/images/front/main13.png'"/></div>
						</td>
                        <td width="186">
                        <div class="r_100"><span class="pointer" onclick="location.href='/doctor/{{attrStr}}'">{{username}}</span><p style=" margin-top:5px;">{{titleNames}}</p></div>
                        </td>
                        <td width="233">
                        	<div class="l_184">
                            	{{consultScope}}
