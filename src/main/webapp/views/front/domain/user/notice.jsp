	...
	<script id="template" type="x-tmpl-mustache">
		...
		{{#rows}}
			<tr height="53" align="center" style="color:#666666">
		        <td width="200" id="content_{{id}}" class="pointer" onclick="lh.alert($('#content_{{id}}').text())">{{title}}{{^title}}{{content}}{{/title}}</td>
				<td width="200" id="reply_{{id}}" class="pointer" onclick="lh.alert($('#reply_{{id}}').text())">{{replyContent}}</td>
				<td width="120">{{receiverUserName}}</td>
		        <td width="155">{{&createDate}}</td>
		        <td width="60" id="statusName_{{id}}">{{&mainStatusName}}</td>
				<td width="100">
					<button class="btn btn-success" onclick="lh.alert($('#reply_{{id}}').text());readMessage({{id}});">查看内容</button>
