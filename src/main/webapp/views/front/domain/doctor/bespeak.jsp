...
<body>
					...
					<div class="t_851_3">
						<table cellpadding="0" cellspacing="0" border="0">
							<tr height="44" valign="middle">
								<td width="46">类型：</td>
								<td width="118"><select class="text_input1" id="typeId">
	...
	<script type="text/javascript" src="/js/front/domain/doctor/bespeak.js"
		title="v"></script>
	<script id="template" type="x-tmpl-mustache">
		<tr height="53" align="center" style="font-size:14px; color:#63a13f">
        	<td width="110">患者名称</td>
            <td width="120">预约时间</td>
            <td width="110">预约类型</td>
 			<td width="110">项目</td>
			<td width="110">检测类型</td>
            <td width="120">状态</td>
            <td width="15">&nbsp;</td>
            <td width="185">操作</td>
        </tr>
		{{#rows}}
			<tr height="58" align="center" style="color:#666666">
		        <td width="110">{{patientName}}</td>
		        <td width="120">{{&bespeakDate}}</td>
		        <td width="110">{{&typeName}}</td>
 				<td width="110">{{&isRegisterAgent}}</td>
				<td width="110">{{&cancerTypesName}}</td>
		        <td width="120">{{&logicStatusName}}</td>
		        <td width="15">&nbsp;</td>
		        <td width="185">
					...
					<button type="button" onclick="updateBespeakRead({{id}});" class="btn btn-success">确认</button>
					<button type="button" onclick="updateBespeaNotRead({{id}});" class="btn btn-danger">驳回</button>
					<button type="button" onclick="bespeakDeatil({{id}});" class="btn btn-success">详情</button>
		        </td>
		    </tr>
		{{/rows}}
	</script>
