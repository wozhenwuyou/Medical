...
<body>
	...
	<script type="text/javascript"
		src="/js/front/domain/doctor/doctorUserReportList.js" title="v"></script>
	...
	<script id="template" type="x-tmpl-mustache">
		...
		{{#rows}}
			<tr height="53" align="center" style="color:#666666">
		        ...
		        <td width="50">{{sexName}}</td>
		        <td width="50">{{age}}{{ageType}}</td>
		        <td width="80">{{patientPhone}}</td>
		        <td width="80">{{itemName}}</td>
		        <td width="80">{{&getAttrDate}}</td>
		        <td width="80">{{&resultHintName}}</td>
 				<td width="70">{{updatedBy}}</td>
  				<td width="80">{{updateDate}}</td>
		        <td width="140">
					<!--<button type="button" onclick="lh.confirm({content: '是否确定删除该病历？', clickYes:deletePatientReport, clickYesParam:{{id}}});" class="btn btn-danger">删除</button>-->
					<button class="btn btn-info" onclick="window.open('/doctor/prdPrint/{{id}}');">打印</button>
