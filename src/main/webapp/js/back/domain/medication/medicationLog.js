/** 基础数据和基础设置 */
lh.config = {
	mainObjLowerName : 'medicationLog',
	mainObjUpperName : 'MedicationLog'
}
	...
	lh.$grid.datagrid({
		...
		[
			...
			{field:'operate',title:'操作',width:120,align:'center',formatter: function(value,row,index){
				//'<span class="opt_alive"><span style="cursor: pointer;color: #EC4949" onclick="openMainObjWin('+index+',\'update\')">修改</span>&nbsp;|&nbsp;'
				return '<span class="update" style="cursor: pointer;color: green" onclick="openMainObjWin('+index+',\'read\')">查看</span></span>'
			...
			{field:'hospitalName',title:'诊所名称',width:260,align:'center'},
			{field:'medicationName',title:'药物名称',width:240,align:'center'},
			{field:'inOrOut',title:'入库或出库',width:120,align:'center',formatter: function(value,row,index){
	        	return value == 1 ? '入库' : '<span style="color:orange">出库</span>';
	        }},
			{field:'inOrOutTime',title:'操作时间',width:120,align:'center',formatter: function(value,row,index){
...
function initFormComponent(){
	...
	$('#f_hospitalId').combobox({
		...
		filter: lh.comboboxDefaultFilter,
		url : '/back/getHospitalArray',
		onSelect:function(rsp){
			var hospitalId = rsp.id;
			//$('#f_medicationId').combobox('reload'); 
			$('#f_medicationId').combobox('reload','/back/getMedicationRepertoryArray?hospitalId='+hospitalId); 
		}
			/*data : [{
			'id' : 1,
			'name' : '出库'
