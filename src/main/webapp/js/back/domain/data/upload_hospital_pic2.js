 ...
 * 需要页面提供：var fileDom = $('#filePaths'); var fileDBIdsDom = $('#fileDBIds');
   上传成功后，将数据保存在：$('#filePaths')，$('#fileDBIds')，UPLOAD_OBJ_2
 */
 
UPLOAD_OBJ_2 = {//有关上传的所有变量或数据都保存在此根对象中
	pathStr:'', //图片路径字符串
	idsStr:'', //图片ID字符串
...
UPLOAD_OBJ_2.initOption = function (option, random){
	if(!option){
		UPLOAD_OBJ_2.option.params.random = random;return;
	}
	var opt = UPLOAD_OBJ_2.option;
	...
	UPLOAD_OBJ_2.option = opt;
}

/**
 * 初始化上传组件的简化方法
 */
UPLOAD_OBJ_2.initUploadSimple = function (option) {
	UPLOAD_OBJ_2.initUpload('/upload', option);//调用完整方法
}
 ...
 * @param {配置对象} option
 */
UPLOAD_OBJ_2.initUpload = function (url, option) {
	if(UPLOAD_OBJ_2.uploader.id){
		UPLOAD_OBJ_2.uploader.destroy();//为了避免重复绑定事件
	}
	if(!url)url = '/upload';
	...
	UPLOAD_OBJ_2.initOption(option, random);//初始化配置参数
	var uploader = new plupload.Uploader({
				browse_button : UPLOAD_OBJ_2.option.chooseBtnDomId,
				url : url,
				//file_data_name : fileName,
				//unique_names:true,
				file_data_name : 'multipartFile',
				multi_selection : UPLOAD_OBJ_2.option.multiFlag,
				...
				multipart_params:UPLOAD_OBJ_2.option.params//额外参数
				
			});
	uploader.init();
	UPLOAD_OBJ_2.upload_events(uploader);//绑定事件
	UPLOAD_OBJ_2.hasInit = true;
}
 ...
 */
UPLOAD_OBJ_2.upload_events = function (uploader){
	var opt = UPLOAD_OBJ_2.option;
	uploader.bind('FilesAdded', UPLOAD_OBJ_2.file_upload_added);
	uploader.bind('UploadProgress', UPLOAD_OBJ_2.file_upload_progress);
	uploader.bind('Error', UPLOAD_OBJ_2.file_upload_error);
	uploader.bind('FileUploaded', UPLOAD_OBJ_2.file_upload_uploaded);
	...
	UPLOAD_OBJ_2.uploader = uploader;
}
...
/** 附件上传：事件 - 添加附件 */
UPLOAD_OBJ_2.file_upload_added = function (uploader, files){
	var html = '';
	if(!UPLOAD_OBJ_2.option.multiFlag){
		if(uploader.files.length > 1){
			if(UPLOAD_OBJ_2.option.multiReplace){
				uploader.removeFile(uploader.files[0]);
				$('#upload_outer_div').empty();
				$('#file_upload_progress').empty();
				$('#filePaths').val('');
				$('#fileDBIds').val('');
				UPLOAD_OBJ_2.pathStr = '';
				UPLOAD_OBJ_2.idsStr = '';
			}else{
	...
	plupload.each(files, function(file) {
		...
		'<div class="upld_container" id="upld_container_'+id+'">'+
			...
			'<div class="upld_ed frontHide" id="upld_ed_'+id+'">'+
				...
				if(UPLOAD_OBJ_2.option.showEdBtns){
					html += 
				...
				}else if(UPLOAD_OBJ_2.option.showDelBtn){
					html += 
					'<button type="button" class="btn btn-danger btn-xs upld_ed_delete" onclick="cancelFile(\''+id+'\')">删除</button>'+
					'<span class="upld_ed_name" id="upld_ed_name_'+id+'">'+name+'：</span><span class="upld_ed_tip">上传成功</span>';
				}else{
		...
		'</div>'
		var fileId = file.id;
		UPLOAD_OBJ_2.fileCollection[fileId] = file;
	});
	if(UPLOAD_OBJ_2.option.showItemDiv){
		$('#upload_outer_div2').append(html);
	}
	if(UPLOAD_OBJ_2.option.autoSubmit){//如果为自动提交，即选择了文件后立即执行上传操作
		uploader.start();
	}
	UPLOAD_OBJ_2.uploader = uploader;
}
...
/** 附件上传：事件 - 上传进度变化 */
UPLOAD_OBJ_2.file_upload_progress = function (uploader,file){
	var id = file.id;
...
UPLOAD_OBJ_2.file_upload_uploaded = function (uploader, file, response){
	UPLOAD_OBJ_2.uploader = uploader;
	...
	if (status && status == 'success') {
		...
		UPLOAD_OBJ_2.file_upload_success(filePath, fileDBId, file);
	}else{
		UPLOAD_OBJ_2.file_upload_failure(rsp.msg);
	}
}

/** 附件上传：事件 - 上传成功 */
UPLOAD_OBJ_2.file_upload_success = function (filePath, fileDBId, file){
	if(UPLOAD_OBJ_2.uploader.files.length <= 1){//第一个附件上传成功后，清除默认路径
		UPLOAD_OBJ_2.pathStr = '';
		UPLOAD_OBJ_2.idsStr = '';
	}
	var fileId = file.id;
	UPLOAD_OBJ_2.pathStr += ','+filePath;
	//UPLOAD_OBJ_2.pathStr = paths;//保存文件路径到变量中
	
	UPLOAD_OBJ_2.idsStr += ','+fileDBId;
	//UPLOAD_OBJ_2.idsStr = fileDBIds;//保存文件ID到变量中
	
	UPLOAD_OBJ_2.pathCollection[fileId] = filePath;
	UPLOAD_OBJ_2.idsCollection[fileId] = fileDBId;
	UPLOAD_OBJ_2.fileCollection[fileId] = file;
	
	if(UPLOAD_OBJ_2.option.successFun){
		UPLOAD_OBJ_2.option.successFun(fileId, filePath);
	}
	
}

/** 附件上传：事件 - 上传失败 */
UPLOAD_OBJ_2.file_upload_failure = function (msg){
	alert("上传出现错误："+msg);
}

/** 附件上传 - 取消上传 */
UPLOAD_OBJ_2.cancelFile = function (fileId){
	var uploader = UPLOAD_OBJ_2.uploader;
	uploader.removeFile(uploader.getFile(fileId));
	$('#upld_container_'+fileId).remove();
	//清除对应附件的路径
	var filePath = UPLOAD_OBJ_2.pathCollection[fileId];
	if(filePath){
		delete UPLOAD_OBJ_2.fileCollection[fileId];
		delete UPLOAD_OBJ_2.pathCollection[fileId];
		delete UPLOAD_OBJ_2.idsCollection[fileId];
		var paths = fileDom.val();
		paths = paths.replace(','+filePath,'');
		UPLOAD_OBJ_2.pathStr = paths;
		
		var idsObj = UPLOAD_OBJ_2.idsCollection;
		var ids = '';
...
/** 附件上传：事件 - 上传失败 */
UPLOAD_OBJ_2.file_upload_error = function (uploader,error){
...
/** 附件上传：事件 - 重置上传组件 */
UPLOAD_OBJ_2.resetUpload = function (){
	$('#file_upload_progress').empty();
	
	UPLOAD_OBJ_2.uploader.files = {};
	UPLOAD_OBJ_2.pathStr = '';
	UPLOAD_OBJ_2.idsStr = '';
	UPLOAD_OBJ_2.fileCollection = {};
	UPLOAD_OBJ_2.pathCollection = {};
	UPLOAD_OBJ_2.idsCollection = {};
