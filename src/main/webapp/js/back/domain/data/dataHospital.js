	...
	lh.$grid.datagrid({
		...
		[
			...
			{field:'operate',title:'操作',width:120,align:'center',formatter: function(value,row,index){
				// onclick="lh.jumpToUrl(\'/back/dataHospitalDetail/'+row.id+'\')">
				return  '<span class="opt_alive"><span style="cursor: pointer;color: green" onclick="showMain(\'诊所数据详情\',\'/back/dataHospitalDetail/'+row.id+'\')">查看详情</span>'
				+'</span>'
				
				+'<span class="opt_trash">'
				/* 去掉彻底删除
				 <span style="cursor: pointer;color: #EC4949;" onclick="lh.commonBatchThoroughDelete('+row.id+')">彻底删除 </span>'
				*/ 
				+'<span style="cursor: pointer;color: green" onclick="lh.commonBatchRecover('+row.id+')">恢复</span></span>';
			}},
			{field:'adminUserName',title:'创建人',width:100,align:'center'},
	        {field:'wholeName',title:'诊所名称',width:220,align:'center'},
	        {field:'hospitalTypeName',title:'诊所类型',width:120,align:'center'},
	        {field:'provinceName',title:'省(市)',width:120,align:'center'},
	        ...
	        {field:'haveVisitValid',title:'资料是否完整',width:100,align:'center',formatter: function(value,row,index){
...
function preAddOrUpdate(mainObj){
	...
	mainObj.haveVisitValid = 1;
	mainObj.isAssignIntention = 1;
	mainObj.isAssignContract = 1;//默认为否
	
	//将省市值保存到localStorage方便下次使用
	if(localStorage){
		localStorage.setItem("data_province", mainObj.province);
		localStorage.setItem("data_city", mainObj.city);
	}
	return true;
}
...
function afterOpenWin(data, operation, isReadOnly){
	if(!data){
		...
		if(localStorage){
			var province = localStorage.getItem("data_province");
			if(province){
				$('#f_province').combobox('select', province);
				 var url = '/back/getCityArray?provinceId='+province;
            	$('#f_city').combobox('reload', url);
			}
			var city = localStorage.getItem("data_city");
			if(city){
				$('#f_city').combobox('setValue', city);
			}
		}
		return;
	}
...
function exportDataHospital(){
	...
	window.location.href = '/back/dataHospitalExcel?'+ $.param( obj );
}