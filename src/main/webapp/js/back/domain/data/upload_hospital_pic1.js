...
UPLOAD_OBJ_1.initOption = function (option, random){
	if(!option){
 ...
 * 初始化上传组件的简化方法
 */
UPLOAD_OBJ_1.initUploadSimple = function (option) {
	UPLOAD_OBJ_1.initUpload('/upload', option);//调用完整方法
}
 ...
 * @param {配置对象} option
 */
UPLOAD_OBJ_1.initUpload = function (url, option) {
	...
	UPLOAD_OBJ_1.initOption(option, random);//初始化配置参数
	...
	uploader.init();
	UPLOAD_OBJ_1.upload_events(uploader);//绑定事件
 ...
 */
UPLOAD_OBJ_1.upload_events = function (uploader){
	var opt = UPLOAD_OBJ_1.option;
...
UPLOAD_OBJ_1.file_upload_uploaded = function (uploader, file, response){
	...
	if (status && status == 'success') {
		...
		UPLOAD_OBJ_1.file_upload_success(filePath, fileDBId, file);
	}else{
		UPLOAD_OBJ_1.file_upload_failure(rsp.msg);
	}
}
...
UPLOAD_OBJ_1.file_upload_success = function (filePath, fileDBId, file){
	...
	var fileId = file.id;
	UPLOAD_OBJ_1.pathStr += ','+filePath;
	//UPLOAD_OBJ_1.pathStr = paths;//保存文件路径到变量中
	
	UPLOAD_OBJ_1.idsStr += ','+fileDBId;
