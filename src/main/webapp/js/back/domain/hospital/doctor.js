/** 基础数据和基础设置 */
lh.config = {
	mainObjLowerName : 'doctor',
	mainObjUpperName : 'Doctor'
}

$(function() {
	loadGrid();
	initQueryComponent();
	$('#opt_outer_div').css('width', (lh.dom.clientSafeWidth-20));
});
...
function loadGrid(){
	lh.$grid.datagrid({
	    ...
	    width:lh.dom.clientSafeWidth-20,
	    height:lh.dom.clientHeight-15,
	    columns:[
		[
			...
			{field:'hospitalName',title:'所属诊所',width:120,align:'center'},
			{field:'departmentName',title:'所属科室',width:120,align:'center'},
	        {field:'username',title:'用户名',width:120,align:'center'},
	        {field:'realname',title:'真实姓名',width:120,align:'center'},
	        {field:'titleName',title:'职称名称',width:120,align:'center'},
	        ...
	        {field:'introduction',title:'执业经历',width:120,align:'center'},
	        {field:'goodAt',title:'擅长',width:120,align:'center'},
...
function initQueryComponent(){
	...
	$('#sc_departmentId').combobox({
		...
		multiple : false,
		panelHeight : 200,
		url : '/back/getDepartmentArray?parentIdNotNull=1'
			/*data : [{
			'id' : 1,
			'name' : '启用'
		},{
...
function preAddOrUpdate(mainObj){
	...
	if(!filePaths){
		...
		if(filePaths.substring(0,1) != "/"){
			...
			ids = ids.substring(1);
			//if(ids == undefined){  //hfq后加
			//	ids = "";            //hfq后加
			//}                      //hfq后加
		}
		mainObj.avatar = filePaths;
		//mainObj.avatarPicId = ids;
		//mainObj.avatarPicId = "";  //hfq后加
	}
	return true;
}
...
function getResetPasswordUrl(){
	return '/back/updateDoctorPassword';
}

function exportDoctor(){
	var obj = lh.getQueryObj();
	delete obj.ascOrdesc;
	window.location.href = '/back/doctorExcel?obj='+obj;
}
