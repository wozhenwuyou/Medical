...
function provinceChange(cityId){
	lh.param = {};
	$("#deptUl a").css('color', 'black');
	$("#hospitalUl a").css('color', 'black');
...
function refreshDoctorList(param){
	...
	$("#deptUl a").css('color', 'black');
	$("#hospitalUl a").css('color', 'black');
	$("#dept_"+departmentId).css('color', '#136FE4');
	$("#hospital_"+hospitalId).css('color', '#136FE4');
	lh.param = param;
	loadGridData();
}
...
function loadGridData(page,size,count){
	...
	if(province && province != '0')obj.province = province;
	if(city && city != '0')obj.city = city;
