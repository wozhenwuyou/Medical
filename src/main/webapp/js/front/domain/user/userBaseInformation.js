...
function freshAge(){
	...
	try{
		...
		var y = nowYear - year;
		var flag = m.add(y, 'year').isAfter(now);
		if(flag)y--;
		$("#age").val(y);
...
function upDatePatientInformation(){
	...
	var obj = {id:userId,username:username,realName:realName,phone:phone,address:address,idcardNum:idcardNum,
			sex:sex,jobId:job,age:age,email:email,birthday:birthday,province:province,city:city};
	if(filePaths){
...
function myReload(){
	window.location.href="/userBaseInformation";
}
