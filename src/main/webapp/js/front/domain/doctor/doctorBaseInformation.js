$(function(){
	highLightLeftMenu('lm_baseInformation');
...
function provinceChange(city){
	...
	lh.post("front", "/getCityArray", {provinceId:provinceId}, function(rsp){
		if(rsp){
			var dom = '<select id="city" style="width: 274px;" class="input8"><option value="">请选择</option>';
			$("#cityDiv").empty();
...
function upDateDoctorInformation(){
	...
	//if(!filePaths || filePaths.length <5){lh.alert('请上传头像');return;}
	var attrStr = $("#attrStr").val();
	if(!attrStr || attrStr.length<5 || attrStr.length >30){lh.alert('请填写固定网址，长度在5-30之间，只能包含字母和数字');return;}
	var obj = {id:doctorId,username:username,realname:realname,idCard:idCard,phone:phone,address:address,email:email,sex:sex,
			province:province,city:city,goodAt:goodAt,introduction:introduction,consultScope:consultScope,titleIds:titleIds,educationCode:educationCode,attrStr:attrStr};
	if(filePaths){
		if(filePaths.substring(0,1) != "/"){
			filePaths = filePaths.substring(1);
		}
		obj.avatar = filePaths;
	}
	/* 
