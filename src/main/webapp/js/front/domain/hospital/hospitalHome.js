...
function loadGridData(page,size,count){
	...
	if(!count)count = 1;
	lh.post('front', '/getDoctorList',{page:page,rows:size,hospitalId:lh.hospitalId},function(rsp){
		if(rsp.success){
...
function getBespeak(){
	if(BESPEAKTYPEID == 1){
		...
		lh.jumpToUrl('/bloodTest/'+lh.hospitalId+'?isBloodTest=1&doctorId='+DOCTORID);
	}
}
