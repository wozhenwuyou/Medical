$(function(){
	highLightLeftMenu('lm_medicationRepertory');
...
function loadGridData(page,size,count){
	...
	if(!count)count = 1;
	var name = $("#name").val();
	var medicationTypeId = $("#medicationTypeId").val();
	if(!name)name = null;
	if(name){
		name = name.trim();
	}
	$.post('/hospital/getMedicationList',{page:page,rows:size,name:name,medicationTypeId:medicationTypeId},function(rsp){
		if(rsp.success){
