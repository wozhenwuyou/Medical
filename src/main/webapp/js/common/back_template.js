...
function mainMenuChange(opt){
	if(opt == 'onCollapse'){
		var px = -1;
	}else if(opt == 'onExpand'){
		var px = 16;
	}
	var width = lh.getClientSafeWidth();
	if(lh.$grid){
		lh.$grid.datagrid('resize', {
  		  	width:width + px
    	});
	}
}
...
function clearSearch(){
	if(beforeClearSearch){//拦截
		var flag = beforeClearSearch();
		if(!flag)return;
	}
	       ...
	       if(flag){
	       		...
	       		lh.post('back', lh.config.addOrUpdateUrl, mainObj, function(rsp){
					 if(rsp.status=='success'){
						 ...
						 var flag = whenAddOrUpdateFailure(rsp);
						 if(!flag){
