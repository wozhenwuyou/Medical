...
public class DiagnoseService extends CommonService<Diagnose> {
	...
	public JSONObject addDiagnose(JSONObject json, Doctor sessionDoctor, Diagnose diagnose) throws ParseException{
		...
		if(Check.isNull(userId)){
			...
			//if(count > 0)return Result.failure(json, "该身份证号码已经存在，请重新输入", "idNum_repeat"); //he注销
			//if(Check.isNull(idCard))return Result.failure(json, "请填写身份证号码", "idNum_null");  //he注销
			
			if(count > 0 && Check.isNotNull(idCard))
				return Result.failure(json, "该身份证号码已经存在，请重新输入", "idNum_repeat");//医生--病历患者身份证为空不验证，he修改添加
