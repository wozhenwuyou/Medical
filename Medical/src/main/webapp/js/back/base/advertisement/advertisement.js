...
function loadGrid(){
	lh.$grid.datagrid({
		...
		[
			...
			{field:'title',title:'图片标题',width:200,align:'center'},
			{field:'linkUrl',title:'链接地址',width:200,align:'center'},
			{field:'catId',title:'栏目位置',width:200,align:'center',formatter: function(value,row,index){
					var catName = '';
					if(value == 1){
						catName = "首页";
					}else if(value == 2){
...
function openMainObj(index, operation){
	...
	if(index >= 0){
		...
		lh.jumpToUrl('/back/addOrUpdateAdvertisementPage?advertisementId='+data.id+"&operation="+operation);
	}
}
