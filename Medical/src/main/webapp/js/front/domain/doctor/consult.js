...
function loadGridData(page,size,count){
	...
	$.post('/getConsultList',{page:page,rows:size,doctorId:doctorId,title:name},function(rsp){
		if(rsp.success){
			var totalNumber = rsp.total || 0;
		    $('#consultList').html(buildDom(rsp.rows));
		    $('#page').empty().Paging({
		    	current:page,
		    	pagesize:size,
		    	count:totalNumber,
...
function consultDetail(id){
	window.location.href = "/consultDetail/"+id;
}
